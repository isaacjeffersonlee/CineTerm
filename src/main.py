        ...
        if mode_choice == 'q':
            ...
            if mode_choice == 's':  # Open the torrent in mpv
                ...
                with Progress() as progress:
                    download_task = progress.add_task(description="[yellow]Waiting to begin download...[/yellow]", total=None)
                    ...
                    progress.update(task_id=download_task,
                                    description=f"[green]Downloading to {buffer_percent*100}%...[/green]",
                    ...
                    while download_info["downloaded"] / download_info["size"] < buffer_percent:
                        download_info = qb.get_torrent_progress(torrent_hash=torrent_hash, login_cookies=login_cookies)
                        progress.update(download_task, completed=download_info["downloaded"])
                        time.sleep(0.5)
